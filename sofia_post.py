"""
Post scheduled images to X (Twitter) for the Sofia Galani account.
Runs via GitHub Actions on a cron schedule.
Posts any row from sofia_schedule.csv whose Greece time has arrived.

This is the Sofia counterpart to x_post.py (Elena). It is identical in
behaviour but reads its own schedule/history/images and authenticates with
the SOFIA_X_* secrets so it posts to a separate X account.
"""
...
IMAGES_DIR = REPO_DIR / "sofia-images"
SCHEDULE_CSV = REPO_DIR / "sofia_schedule.csv"
HISTORY_CSV = REPO_DIR / "sofia_history.csv"
...
def load_config():
    keys = ["SOFIA_X_API_KEY", "SOFIA_X_API_SECRET",
            "SOFIA_X_ACCESS_TOKEN", "SOFIA_X_ACCESS_TOKEN_SECRET"]
    config = {}
    for k in keys:
...
def read_schedule():
    if not SCHEDULE_CSV.exists():
        print("ERROR: sofia_schedule.csv not found")
        sys.exit(1)
...
def process_posts():
    ...
    auth = tweepy.OAuth1UserHandler(
        config["SOFIA_X_API_KEY"],
        config["SOFIA_X_API_SECRET"],
        config["SOFIA_X_ACCESS_TOKEN"],
        config["SOFIA_X_ACCESS_TOKEN_SECRET"],
    )
    api_v1 = tweepy.API(auth)
    client_v2 = tweepy.Client(
        consumer_key=config["SOFIA_X_API_KEY"],
        consumer_secret=config["SOFIA_X_API_SECRET"],
        access_token=config["SOFIA_X_ACCESS_TOKEN"],
        access_token_secret=config["SOFIA_X_ACCESS_TOKEN_SECRET"],
    )
    ...
    if changed:
        ...
        # Append posted rows to sofia_history.csv
        if posted_rows:
            ...
            with open(HISTORY_CSV, "a", newline="", encoding="utf-8") as f:
                ...
                for row in posted_rows:
                    ...
                    writer.writerow(row)
            print(f"  Added {len(posted_rows)} row(s) to sofia_history.csv")
